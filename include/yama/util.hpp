...
public:
    ...
    static constexpr T PI_HALF = T(1.5707963267948966192313216916398);
    static constexpr T PI_D4 = T(0.78539816339744830961566084581988); // pi / 4
    static constexpr T PI_DBL = T(6.283185307179586476925286766559); // 2*pi
    static constexpr T OVER_PI = T(0.31830988618379067153776752674503); // 1/pi
    static constexpr T E = T(2.71828182845904523536028747135266249); //natural constant
    static constexpr T SQRT_2 = T(1.4142135623730950488016887242097); //sqrt(2)

    static constexpr T EPSILON = T(1e-5); //epsilon for floating point equalities
    static constexpr T EPSILON_LOW = T(1e-3); //low precision epsilon
    static constexpr T EPSILON_HIGH = T(1e-7); //high precision epsilon
};
